...
@Autonomous(name="AutoRedDuckOld", group="Auto")
@Disabled
public class AutoRedDuckOld extends LinearOpMode {
    ...
    public void GoTo(double tox, double toy, float rad, double spd, double timeout, int strela, String style) {
        ...
        while ((!done | !stdone) & opModeIsActive() & runtime.seconds() < timeout) {
            ...
            accel = (laststyle == "continue") ? 1 : Range.clip(runtime.milliseconds() / 350, 0.8, 1);
            antiaccel = Range.clip(c / 20, 0.2, 0.5);
            ...
            switch (style) {
                ...
                case "continue":
                    ...
                    if (laststyle == "element") {
                        m5.setPower(zm5 * (Math.round(runtime.milliseconds() / 400) % 2 == 0 ? 1 : 0));
                    }
                    break;
            ...
            case "duck":
                m5.setPower(zm5 * (Math.round(runtime.milliseconds() / 100) % 2 == 0 ? 1 : 0));
                // обычная езда
                if (c > 0) {
                    m1.setPower(spd * (zm1 + cor));
                    m2.setPower(spd * (zm2 + cor));
    ...
    public void runOpMode() throws InterruptedException {
        ...
        GoTo(25, 46, -47, 0.45, 2.5, baza, "stop"); // порт
        ...
        GoTo(-26, -6, -90, 0.6, 2.5, 1, "stop"); // колесо
        m7.setPower(-0.55); // вращение колеса
        GoTo(-30, -9, -90, 0.3, 5, 1, "continue"); // прижим к колесу
        m7.setPower(0);
        GoTo(-26, 0, 0, 0.3, 2, 1, "stop");
        GoTo(-26, 6, 0, 0.3, 2, 1, "stop");
        GoTo(-13, 6, 0, 0.3, 2, 1, "duck");
        GoTo(-13, 3, 0, 0.2, 2, 1, "duck"); // сбор утки

        
        GoTo(25, 46, -47, 0.45, 2.5, 3, "stop"); // порт
        ...
        GoTo(20, 26, 90, 0.6, 1.5, 1, "stop"); // к стенке (поворот)
        GoTo(20, -8, 90, 0.6, 2, 1, "stop"); // к стенке
        GoTo(140, -10, 90, 0.85, 2, 1, "stop"); // парковка
        GoTo(146, -10, 90, 0.5, 2, 1, "element"); //сбор элемента
